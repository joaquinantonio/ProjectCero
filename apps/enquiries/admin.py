...
class EnquirySubmissionAdmin(
    ...
    search_help_text = (
        "Search by reference, name, email, phone, subject, related merch, "
        "message, or admin notes."
    )
    ordering = ("-created_at",)
        ...
        return (
            (
                "Workflow",
                {
                    ...
                    "description": (
                        ...
                        "The original submission details are shown below."
                    ),
                },
            ),
            (
            ...
            (
                "Enquiry Details",
                {
                    ...
                    "description": (
                        "Submitted enquiry details. Related event or merch item "
                        "can be linked internally if needed."
                    ),
                },
            ),
            (
    ...
    def get_form(self, request, obj=None, **kwargs):
        ...
        if "status" in form.base_fields:
            form.base_fields["status"].help_text = (
                "Use this to track the enquiry as it moves through your workflow."
            )
        ...
        if "preferred_start_time" in form.base_fields:
            ...
            form.base_fields["preferred_start_time"].help_text = (
                "Optional. In 30-minute increments."
            )

        if "amount_text" in form.base_fields:
            form.base_fields["amount_text"].label = "Amount / package"
            form.base_fields["amount_text"].help_text = (
                "Optional. For merch enquiries, this can store the displayed item price."
            )

        if "quantity" in form.base_fields:
            form.base_fields["quantity"].label = "Requested quantity"
            form.base_fields["quantity"].help_text = (
                "Used mainly for merch purchase enquiries."
            )
...
class ArtistEnquiryAdmin(
    ...
    search_help_text = (
        "Search by reference, name, email, phone, artist name, or admin notes."
    )
    ordering = ("-created_at",)
    date_hierarchy = "created_at"
        ...
        return (
            (
                "Workflow",
                {
                    ...
                    "description": (
                        "Update the status and internal notes here. "
                        "Contact the sender using their phone number via WhatsApp or call."
                    ),
                },
            ),
            (
    ...
    def get_form(self, request, obj=None, **kwargs):
        ...
        if "status" in form.base_fields:
            form.base_fields["status"].help_text = (
                "Mark as 'Contacted' after reaching out via WhatsApp/call. "
                "Mark as 'Closed' once complete."
            )

        if "admin_notes" in form.base_fields:
            form.base_fields["admin_notes"].label = "Internal notes"
            form.base_fields["admin_notes"].help_text = (
                "Visible only in admin. Record outcomes of contact attempts."
            )
